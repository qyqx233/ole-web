...
async function getResponse(instance: AxiosInstance, options: any, retry = 0): Promise<AxiosResponse<any>> {
...
function parseDetailId(link: string) {
  return link && link.replace(/\/index\.php\/vod\/detail\/id\/|.html/g, '')
};

function parsePlayId(link: string) {
...
class Olevod {
   ...
   */
  static getVideos() {
    const { search, typeId, page = 1, order = c.ORDERS.TIME.value, year, letter,
      area, language, condition, instance = olevodInstance }
  ...
  const  url = `${c.BASE_URL}/index.php/vod/${vod}.html`;
  console.log(params);
  console.log(url)
  const options = {
    method: 'GET',
